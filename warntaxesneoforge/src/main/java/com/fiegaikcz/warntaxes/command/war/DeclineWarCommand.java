...
public class DeclineWarCommand {

    public static LiteralArgumentBuilder<CommandSourceStack> register() {
        return Commands.literal("declinewar")
                .then(Commands.argument("target", net.minecraft.commands.arguments.EntityArgument.player())
                        .executes(DeclineWarCommand::execute));
    }
    ...
    private static int execute(CommandContext<CommandSourceStack> context) {
        ...
        if (!isAuthorized) {
            context.getSource().sendFailure(Component.literal("Only the colony owner or officer can decline war."));
            return 0;
        }
        ...
        if (!ColonyWarManager.hasProposal(idA, idB)) {
            context.getSource().sendFailure(Component.literal("There is no war proposal to decline."));
            return 0;
        }

        Integer proposedGuards = ColonyWarManager.getProposedGuards(idA, idB);
        if (proposedGuards != null) {
            sourcePlayer.sendSystemMessage(Component.literal("They proposed war with " + proposedGuards + " committed guards."));
        }

        ColonyWarManager.declineWar(idA, idB);

        sourcePlayer.sendSystemMessage(Component.literal("You declined the war proposal."));
        targetPlayer.sendSystemMessage(Component.literal(sourcePlayer.getName().getString() + " declined your war proposal."));
